...
func (r *PgCharacterRepository) Create(ctx context.Context, character domain.Character) error {
	const query = `
		INSERT INTO characters (id, clone_profile_id, name, relation, archetype, bond_status, bond_level, created_at, updated_at)
...
func (r *PgCharacterRepository) Update(ctx context.Context, character domain.Character) error {
	const query = `
		UPDATE characters
		SET name = $1, relation = $2, archetype = $3, bond_status = $4, bond_level = $5, updated_at = $6
		WHERE id = $7
	`
	_, err := r.pool.Exec(ctx, query,
		character.Name,
...
func (r *PgCharacterRepository) ListByProfileID(ctx context.Context, profileID uuid.UUID) ([]domain.Character, error) {
	const query = `
		SELECT id, clone_profile_id, name, relation, archetype, bond_status, bond_level, created_at, updated_at
		FROM characters
		WHERE clone_profile_id = $1
		ORDER BY bond_level DESC, name ASC
	`
